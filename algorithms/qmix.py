...
from networks.qmixnet import QmixNet
...
class Qmix(object):
    ...
    def __init__(self, args):
        ...
        self.eval_mixnet = QmixNet(self.state_shape, self.hidden_shape, self.n_agents, self.device)
        self.target_mixnet = QmixNet(self.state_shape, self.hidden_shape, self.n_agents, self.device)
    ...
    def update(self, buffer, episode):
        ...
        for data_batch in data_batchs:
            ...
            q_eval_sum = self.eval_mixnet(state[:, :-1], q_eval[:, :-1])
            q_target_sum = self.target_mixnet(state[:, 1:], q_target[:, 1:])
    ...
    def save(self, model_path):
        torch.save(self.target_net.state_dict(), model_path + '/qmix_q_params.pkl')
        torch.save(self.target_mixnet.state_dict(), model_path + '/qmix_mix_params.pkl')
